...
CShape g_Shape;
cv::Mat g_pImgData16[20];   // 缓存的16位数据
int g_iImgIndex;
									...
									float* pVecDis, PairDis* pVecPairDis, float* pVecAngs, ZYResultDataMin* ResData,int iMethode,int imagNo)
{
	...
	if (3 == input.iMethode)
	{
		...
		g_pImgData16[imagNo % 20].copyTo(input.Src);*/

		//TODO: 替换算法接口  ZhangKF 2021-3-16  
		input.Src = cv::Mat::zeros(cvSize(image->height, image->width), CV_16UC1);
		cv::Mat TempImage16 = cv::Mat(image->height, image->width, CV_16UC1, image->data);

		//保存收到的图像
		//string path = "e:\\";
		//string ext = ".tiff";
		//string coreIndex = to_string(params->iCorner;)
		//string fileName = path + barcode + coreIndex + ext;
		//cv::imwrite(fileName, TempImage16);

		TempImage16.copyTo(input.Src);
	}
	else
	{
...
void strReplace(char* s1,char* s2, char* str1, char* str2)
{
	char *pos0, *pos1, *pos2;
...
int ImageProcessNativeIF_SaveImage(const ZYImageStruct* image, char* filePath, int iType,int imageNo)
{
	...
	try
	{
		...
		if (1 == iType)
		{
			char filePath16[1024];
			strReplace(filePath, filePath16, ".jpg", ".tiff");
			cv::imwrite(filePath16, g_pImgData16[imageNo % 20]);
		}
	}
