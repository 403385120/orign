...
namespace ZYXray.ViewModels
{
    public class MotionControlVm : ObservableObject
    {
        ...
        //TODO:对多线程同时访问的dicBarcode2加锁 zxh 2021-04-12
        private object scanobj = new object();
        ...
        private static Queue<BatterySeat> queueSortingStation = new Queue<BatterySeat>();
        ...
        SocketTCPInterface tcpScan = null;
        SocketTCPInterface tcpUnScan = null;
        public static SocketTCPInterface CreateSocket(string ip, int port)
        {

            Communication _m = new Communication();
            Encoding encoding = Encoding.ASCII;

            SocketTCPInterface tcpPlc = new SocketTCPInterface(encoding, ip, port, _m.IpLocalPort.GetValueOrDefault(-1));
            tcpPlc.BufferSize = 1;
            ...
            tcpPlc.IsLine = _m.IsLine.GetValueOrDefault(true);
            ...
            tcpPlc.SendCommand1 = "LON";

            tcpPlc.SendCommand2 = "LOFF";
        ...
        private List<DateTime> listPPGOutTime = new List<DateTime>();
        ...
        private bool isBarcodeBinding2Finished = true;
        private bool isOcv2Finished = true;

        //TODO: 增加对变量的限制以便多线程准确访问 ZhangKF 2021-4-7
        //private static bool isPPG2Finished = true;
        private static volatile bool isPPG2Finished = true;
        ...
        //TODO:厚度线性点检是否通过变量 ZhangKF 2021-4-5
        ///<summary>厚度线性点检是否通过</summary>
        private bool canDoPPGLinear = false;
        private bool canDoXRAY = false;
        private bool canDoXRAYNeedle = false;

        //TODO:改善弹出框对线程的卡住导致的超时问题，解决方式为完成信号归零后再弹出 ZhangKF 2021-4-9
        private volatile bool _MessageBox_Same_Exception_MDIComplete1 = false;
        private string _MessageBox_Msg_MDIComplete1 = string.Empty;
        private volatile bool _MessageBox_Same_Exception_MDIComplete2 = false;
        private string _MessageBox_Msg_MDIComplete2 = string.Empty;
        //End
        ...
        /// <summary>
        /// 工艺路线
        /// <para>0: OB - XRay - FQI</para>
        /// <para>1: OC - XRay - FQI</para>
        /// <para>2: O4 - XRay - FQI(W区)</para>
        /// <para>3: XRay - FQI</para>
        /// <para>4: O4 - FQI (XRay工序已OK，不需要人工复判)</para>
        /// <para>5: OG - FQI (XRay工序已OK，不需要人工复判)</para>
        /// <para>6: FQI (XRay工序已OK，不需要人工复判)</para>
        /// <para>7: OB-XRAY </para>
        /// <para>8: OCVG </para>
        /// <para>9: 离线生产</para>
        /// </summary>
        ...
        public static SerialPort InitCom(SerialPort Serial, string PortName = "COM1", int BaudRate = 115200, int DataBits = 8, Parity Parity = Parity.None, StopBits StopBits = StopBits.One)
        {
            if (Serial == null)
            {
        ...
        public bool InitialE5CC(string PortName = "COM1", int BaudRate = 9500, int DataBits = 8, Parity Parity = Parity.None, StopBits StopBits = StopBits.One)
        {
            _e5CCSerialPort = InitCom(_e5CCSerialPort, PortName, BaudRate, DataBits, Parity, StopBits);
        ...
        public double GetE5CCTemperatrue()
        {
            if (E5cc != null)
            {
                ...
                try
                {
                    _e5CCSerialPort.Write(array, 0, array.Length);
                    Thread.Sleep(150);
                    int bytesToRead = _e5CCSerialPort.BytesToRead;
                    byte[] array2 = new byte[bytesToRead];
                    if (array2.Length != 0)
                    {
                        _e5CCSerialPort.Read(array2, 0, bytesToRead);
                        StringBuilder stringBuilder = new StringBuilder();
                        int num2;
                        for (int num = 0; num < array2.Length; num = num2 + 1)
                        {
                            stringBuilder.AppendFormat("{0:x2} ", array2[num]);
                            num2 = num;
                        }
                        list = stringBuilder.ToString().Split(' ').ToList();
                        for (int i = 5; i < list.Count - 3; i++)
                        {
                            list2.Add((double)(Convert.ToInt32(list[i], 16) * 256 + Convert.ToUInt32(list[i + 1], 16)) / 10.0);
                        }
                        result = list2[0];
                    }
                    else
        ...
        public MotionControlVm()
        {
            ...
            //InitialE5CC(HardwareConfig.Instance.E5CCSerialPortConfig.PortName, HardwareConfig.Instance.E5CCSerialPortConfig.BaudRate, HardwareConfig.Instance.E5CCSerialPortConfig.DataBits, (Parity)HardwareConfig.Instance.E5CCSerialPortConfig.Parity, (StopBits)HardwareConfig.Instance.E5CCSerialPortConfig.StopBits);


            //double data = GetE5CCTemperatrue();············
            ...
            SoftWareResetThread();
            HeartBeatThread();
            tcpScan = CreateSocket(HardwareConfig.Instance.ScanBarcodeIPAdress, HardwareConfig.Instance.ScanBarcodePort);
            tcpUnScan = CreateSocket(HardwareConfig.Instance.ScanBarcodeIPAdress2, HardwareConfig.Instance.ScanBarcodePort2);
        ...
        public void LoadWorkParams(CheckParamsConfig checkParams, CameraParams camParam1, CameraParams camParam2)
        {
            ...
            _workingSeats._seat2.Corner1.InspectParams.pixel_to_mm = _camParam2.pixelRatio;
            _workingSeats._seat2.Corner2.InspectParams.pixel_to_mm = _camParam2.pixelRatio;

            _workingSeats._seat2.Corner3.InspectParams.pixel_to_mm = _camParam1.pixelRatio;
            _workingSeats._seat2.Corner4.InspectParams.pixel_to_mm = _camParam1.pixelRatio;
            //
            ...
            _workingSeats._seat1.Corner1.InspectParams.pixel_to_mm = _camParam2.pixelRatio;
            _workingSeats._seat1.Corner2.InspectParams.pixel_to_mm = _camParam2.pixelRatio;

            _workingSeats._seat1.Corner3.InspectParams.pixel_to_mm = _camParam1.pixelRatio;
            _workingSeats._seat1.Corner4.InspectParams.pixel_to_mm = _camParam1.pixelRatio;
        ...
        string BarcodeLenth = string.Empty;
            ...
            get
            {
                return new RelayCommand(new Action(delegate
                {
                    if (Common.IsShortSocket)
                    {
                        ...
                        System.Windows.MessageBox.Show("当前模式为短连接，不允许生产，请切换为长连接再进行生产", "提示", System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.None,
                            System.Windows.MessageBoxResult.OK, System.Windows.MessageBoxOptions.ServiceNotification);
                        return;
                    }
                    else
                    {
                        if (Bt3562 == null)//短连接测试之后默认为null，需要重新连接
                        {
                            try
                            {
                            ...
                            {
                                System.Windows.MessageBox.Show(ex.ToString(), "提示", System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.None,
                           System.Windows.MessageBoxResult.OK, System.Windows.MessageBoxOptions.ServiceNotification);
                            }
                        }
                    ...
                    //string errMsg = string.Empty;
                    //string url = " http://atlmes-w.atlbattery.com/Apriso/HttpServices/api/extensions/1/cell/ExistsSerialNo2";
                    //var processData = new
                    //{
                    //    InputTypes = "",
                    //    OutputTypes = "",
                    //    OperationCode = "TestOperation",
                    //    OperationRevision = "",
                    //};
                    //var jsonData = JsonConvert.SerializeObject(processData);
                    //var resultMsg = Esquel.WebApiService.WebApiPostHelper.Post(ref errMsg, url, jsonData, "json", false);
                    ...
                    UserDefineVariableInfo.DicVariables["CCDVersion"] = "3.0.2.6";

                    plc.WriteByVariableName("SoftwareOnState", 1);
                    frmAtlLogin frm = new frmAtlLogin();
                    if (frm.ShowDialog() == DialogResult.OK)
                    {
                        ReworkMode = frm.Model;
                        BarcodeLenth = frm.BarcodeLenth;
                    }
                    else
                    {
                        return;
                    ...
                    {
                        System.Windows.MessageBox.Show("算法未初始化完成!", "提示", System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.None, System.Windows.MessageBoxResult.OK, System.Windows.MessageBoxOptions.ServiceNotification);
                        return;
                    }
                    ...
                    if (ReworkMode == 0)
                        ...
                        workroute = "O4 - XRay - FQI";
                    else if (ReworkMode == 3)
                        workroute = "XRay - FQI";
                    else if (ReworkMode == 4)
                        workroute = "O4 - FQI";
                    else if (ReworkMode == 5)
                        workroute = "OG - FQI";
                    else if (ReworkMode == 6)
                        workroute = "FQI";
                    else if (ReworkMode == 7)
                        workroute = "OB - XRAY";
                    else if (ReworkMode == 8)
                        workroute = "OCVG";
                    else if (ReworkMode == 9)
                        workroute = "离线生产";
                    ...
                    if (_AutoRunning == true)
                    {
                        ...
                        ThreadPool.QueueUserWorkItem(state => Task_UnLoadScan());
            ...
            new Thread(() =>
            {
                ...
                try
                {
                    ...
                    while (_AutoRunning == true)
                    {
                        ...
                        if (ReworkMode != 9)//isOpenBis == true
                        {
                            ...
                            //TODO: 厚度线性点检是否通过 ZhangKF 2021-4-5
                            this.canDoPPGLinear = CheckCanDo(CheckParamsConfig.Instance.LinearCheckTime);
                            ...
                            if (CheckCanDo(CheckParamsConfig.Instance.NeedleCheckTime))
                            {
                                canDoXRAYNeedle = true;
                            }
                            else
                            {
                                canDoXRAYNeedle = false;
                            }
                        }
                        else
                        {
        ...
        public void SoftWareReset()
        {
            ...
            while (queueSortingStation.Count > 0)
            {
                queueSortingStation.Dequeue().Destroy();
            }
                ...
                switch (MotionEnum.EnumScanBarcode1)
                {
                    case MotionEnum.ScanBarcode1.工位1触发扫码:
                        ...
                        if (plc.ReadByVariableName("ScanBarcodeSignal1") == "1" && plc.ReadByVariableName("TestMode") == "1")
                        {
                            ...
                            string errmsg = string.Empty;
                            if (!tcpScan.IsClose && !tcpScan.Connected)
                            {
                                bool isOpen = InitSocketConnect(tcpScan, ref errmsg);
                                if (!isOpen)
                                {
                                    WriteLog(string.Format("1.前扫码无法连接，请检查扫码枪{0}", errmsg), LogLevels.Info, "ScanBarcodeGRR");
                                    continue;
                                }
                            }
                            string errMsg = string.Empty;
                            var inputData = $"{tcpScan.SendCommand1}{tcpScan.EndString}";
                            var inputDataClear = $"{tcpScan.SendCommand2.ToUpper()}{tcpScan.EndString}";
                            barcode = tcpScan.SendData(inputData, ref errMsg, true, tcpScan.CommDelayIngTime, tcpScan.IsWriteLog).Replace("\r", "").Replace("\n", "");
                            if (!string.IsNullOrEmpty(errMsg) || barcode.Contains("ERROR"))
                            {
                                ScanNG++;
                                if (ScanNG < tcpScan.ScanCount)
                                {
                                    WriteLog(string.Format("2. {0}", errMsg), LogLevels.Info, "Task_ScanBarcode2");
                                    continue;//扫码失败3次执行下一步
                                }
                                barcode = tcpScan.SendData(inputDataClear, ref errMsg, true, tcpScan.CommDelayIngTime, tcpScan.IsWriteLog);
                                if (errMsg.Contains("由于目标计算机积极拒绝"))
                                {
                                    WriteLog(string.Format("3. 前扫码无法连接，请检查扫码枪{0}", errMsg), LogLevels.Info, "ScanBarcodeGRR");
                                    continue;
                                }
                            }
                    ...
                    case MotionEnum.ScanBarcode2.工位2触发扫码:
                        ...
                        if (plc.ReadByVariableName("ScanBarcodeSignal2") == "1" && plc.ReadByVariableName("TestMode") == "1")
                        {
                            ...
                            if (!tcpScan.IsClose && !tcpScan.Connected)
                            {
                                ...
                                if (!isOpen)
                                {
                                    WriteLog(string.Format("1.第{0}次 前扫码无法连接，请检查扫码枪{1}", index, errmsg), LogLevels.Info, "Task_ScanBarcode2");
                                    continue;
                                }
                            }
                            string errMsg = string.Empty;
                            ...
                            if (!string.IsNullOrEmpty(errMsg) || barcode.Contains("ERROR"))
                            {
                                ...
                                if (ScanNG < tcpScan.ScanCount)
                                {
                                    WriteLog(string.Format("2.第{0}次 {1}", index, errMsg), LogLevels.Info, "Task_ScanBarcode2");
                                    continue;//扫码失败3次执行下一步
                                }
                                barcode = tcpScan.SendData(inputDataClear, ref errMsg, true, tcpScan.CommDelayIngTime, tcpScan.IsWriteLog);
                                if (errMsg.Contains("由于目标计算机积极拒绝"))
                                {
                                    WriteLog(string.Format("3.第{0}次 前扫码无法连接，请检查扫码枪{1}", index, errMsg), LogLevels.Info, "Task_ScanBarcode2");
                                    continue;
                                }
                            }
                            ...
                            if (barcode != string.Empty && !barcode.Contains("ERROR"))
                            {
                                ...
                                if (Common.IsCheckTwoBarcode)
                                {
                                    if (arrBarcode.Length > 1)
                                    {
                                        string barcode1 = arrBarcode[0].Replace("+", "_");
                                        string barcode2 = arrBarcode[1];
                                        string msg = HttpGet("http://atlmes-w.atlbattery.com/Apriso/HttpServices/api/extensions/1/cell/ExistsSerialNo2/" + barcode1 + "/" + barcode2);
                                        if (msg.Contains("true"))
                                        {
                                            WriteLog(string.Format("第{0}次 {1} 条码为 {2},二次码上传校验成功", index, strStep, barcode), LogLevels.Info, "Task_ScanBarcode2");
                                        }
                                        else
                                        if (msg.Contains("false"))
                                        {
                                            plc.WriteByVariableName("BatteryScanning2NG", 1);
                                            WriteLog(string.Format("第{0}次 {1} 条码为 {2},二次码上传校验失败", index, strStep, barcode), LogLevels.Info, "Task_ScanBarcode2");
                                        ...
                                        {
                                            plc.WriteByVariableName("BatteryScanning2NG", 1);
                                            System.Windows.MessageBox.Show("访问url异常，请检查或者关闭二次码上传" + msg);
                                            WriteLog(string.Format("第{0}次 {1} 条码为 {2},访问url异常，请检查或者关闭二次码上传", index, strStep, barcode), LogLevels.Info, "Task_ScanBarcode2");
                                            MotionEnum.EnumScanBarcode2 = MotionEnum.ScanBarcode2.工位2扫码完成;
                                            continue;
                                        }
                                        else
                                        {
                                            plc.WriteByVariableName("BatteryScanning2NG", 1);
                                            WriteLog(string.Format("第{0}次 {1} 条码为 {2},访问url异常，请检查或者关闭二次码上传{3}---{4}", index, strStep, barcode, msg, "http://atlmes-w.atlbattery.com/Apriso/HttpServices/api/extensions/1/cell/ExistsSerialNo2/" + barcode1 + "/" + barcode2), LogLevels.Info, "Task_ScanBarcode2");
                                            MotionEnum.EnumScanBarcode2 = MotionEnum.ScanBarcode2.工位2扫码完成;
                                            continue;
                                        }
                                    }
                                ...
                                //条码前缀校验
                                if (Common.IsCheckBarcodeLenth)
                                {
                                    string[] arr = barcode.Split('+');
                                    string splitBarcode = string.Empty;
                                    if (arr.Length > 1)
                                    {
                                        splitBarcode = arr[1];
                                    }
                                    ...
                                    string lenthBarcode = splitBarcode.Substring(0, BarcodeLenth.Length);
                                    if (lenthBarcode != BarcodeLenth)
                                    {
                                        System.Windows.MessageBox.Show($"条码前缀{lenthBarcode}与设置{BarcodeLenth}的不一致!", "提示",
                                  System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.Information, System.Windows.MessageBoxResult.OK, System.Windows.MessageBoxOptions.ServiceNotification);
                                        WriteLog(string.Format("第{0}次 {1} 条码为 {2} 前缀设置的不一致", index, strStep, barcode), LogLevels.Warn, "Task_ScanBarcode2");
                                        plc.WriteByVariableName("BatteryScanning2NG", 1);
                                        MotionEnum.EnumScanBarcode2 = MotionEnum.ScanBarcode2.工位2扫码完成;
                                        continue;
                                    }
                                ...
                                //TODO: 把扫到的条码给到PLC，由于PLC字符串显示的高低位与上位机相反，在这里做下处理（上位机"1234"->PLC"2143"） zxh 2021 - 04 - 19
                                string revecode = UpLowReverse(barcode);
                                plc.WriteByVariableName("BeforeBarcode", revecode);
                                WriteLog(string.Format("第{0}次 写入上料条码数据 {1}，处理后 {2}", index, barcode, revecode), LogLevels.Info, "Task_ScanBarcode2");
            ...
            while (true)
            {
                ...
                switch (MotionEnum.EnumBarcodeBinding1)
                {
                        ...
                        if (plc.ReadByVariableName("BarcodebindingSignal1") == "1" && plc.ReadByVariableName("TestMode") == "1")
                        {
                            ...
                            if (plc.ReadByVariableName("SingleScanerMode") == "1")//单扫码枪模式
                            {
                                ...
                                if (dicBarcode2.ContainsKey(tempBarcode))
                                {
                                    ...
                                    {
                                        WriteLog("条码1绑定异常" + ex, LogLevels.Error, "Task_BarcodeBinding1");
                                        MessageBox.Show("条码绑定1异常");
                                        index = -2;
                                        continue;
                                    }
                    ...
                    case MotionEnum.BarcodeBinding1.条码绑定完成:
                        ...
                        if (plc.ReadByVariableName("BarcodebindingSignal1") == "0" && isBarcodeBinding2Finished == true)
                        {
            ...
            while (true)
            {
                ...
                switch (MotionEnum.EnumBarcodeBinding2)
                {
                    case MotionEnum.BarcodeBinding2.触发条码绑定:
                        ...
                        {
                            isBarcodeBinding2Finished = false;
                            Thread.Sleep(300);
                            tickCount = Environment.TickCount;
                            index += 2;
                            ...
                            if (dicBarcode2.ContainsKey(tempBarcode))
                            {
                                ...
                                {
                                    WriteLog("条码2绑定异常" + ex, LogLevels.Error, "Task_BarcodeBinding2");
                                    MessageBox.Show("条码绑定2异常");
                                    index = -2;
                                    continue;
                                }
                                bt.TempBarcode = tempBarcode;
                    ...
                    case MotionEnum.BarcodeBinding2.条码绑定完成:
                        ...
                        {
                            isBarcodeBinding2Finished = true;
                            WriteLog(string.Format("第{0}次 {1} ,共耗时 {2}", index, strStep, Environment.TickCount - tickCount), LogLevels.Info, "Task_BarcodeBinding2");
                            plc.WriteByVariableName("BarcodeBindingComplete2", 0);
                            MotionEnum.EnumBarcodeBinding2 = MotionEnum.BarcodeBinding2.触发条码绑定;
                        }
        ...
        /// 下料扫码
        /// </summary>
        private void Task_UnLoadScan()
        {
            MotionEnum.EnumUnLoadScan = MotionEnum.UnLoadScan.下料扫码触发;
            plc.WriteByVariableName("BatteryScanning1OK", 0);
            plc.WriteByVariableName("BatteryScanning1NG", 0);
            WriteLog("下料扫码线程开启", LogLevels.Info, "Task_UnLoadScan");
            string strStep = "";
            int ScanNG = 0;
            int index = 0;
            int ngcounter = 0;
            while (true)
            {
                ...
                if (_AutoRunning == false)
                {
                    WriteLog("下料扫码线程退出", LogLevels.Info, "Task_UnLoadScan");
                    return;
                }
                if (plc.ReadByVariableName("SoftwareReset") == "1")
                {
                ...
                strStep = Enum.GetName(typeof(MotionEnum.UnLoadScan), MotionEnum.EnumUnLoadScan);

                switch (MotionEnum.EnumUnLoadScan)
                {
                    case MotionEnum.UnLoadScan.下料扫码触发:

                        if (plc.ReadByVariableName("ScanBarcodeSignal1") == "1" && plc.ReadByVariableName("TestMode") == "1")
                        {
                            if (queueSortingStation.Count == 0)
                            {
                                WriteLog("下料扫码工位无料!", LogLevels.Warn, "Task_UnLoadScan");
                                Thread.Sleep(3000);
                                continue;
                            }
                            index = queueSortingStation.Peek().TempBarcode;
                            Thread.Sleep(HardwareConfig.Instance.ScanBarcodeDelay);
                            #region 短连接扫码

                            string barcode = string.Empty;
                            string errmsg = string.Empty;
                            if (!tcpUnScan.IsClose && !tcpUnScan.Connected)
                            {
                                bool isOpen = InitSocketConnect(tcpUnScan, ref errmsg);
                                if (!isOpen)
                                {
                                    WriteLog(string.Format("1.第{0}次 后扫码无法连接，请检查扫码枪{1}", index, errmsg), LogLevels.Info, "Task_UnLoadScan");
                                    continue;
                                }
                            }
                            string errMsg = string.Empty;
                            var inputData = $"{tcpUnScan.SendCommand1}{tcpUnScan.EndString}";
                            var inputDataClear = $"{tcpUnScan.SendCommand2.ToUpper()}{tcpUnScan.EndString}";
                            barcode = tcpUnScan.SendData(inputData, ref errMsg, true, tcpUnScan.CommDelayIngTime, tcpUnScan.IsWriteLog).Replace("\r", "").Replace("\n", "");
                            if (!string.IsNullOrEmpty(errMsg) || barcode.Contains("ERROR"))
                            {
                                ScanNG++;
                                if (ScanNG < tcpUnScan.ScanCount)
                                {
                                    WriteLog(string.Format("2.第{0}次 {1}", index, errMsg), LogLevels.Info, "Task_UnLoadScan");
                                    continue;//扫码失败3次执行下一步
                                }
                                barcode = tcpUnScan.SendData(inputDataClear, ref errMsg, true, tcpUnScan.CommDelayIngTime, tcpUnScan.IsWriteLog);
                                if (errMsg.Contains("由于目标计算机积极拒绝"))
                                {
                                    WriteLog(string.Format("3.第{0}次 前扫码无法连接，请检查后扫码枪{1}", index, errMsg), LogLevels.Info, "Task_UnLoadScan");
                                    continue;
                                }
                            }
                            ...
                            WriteLog(string.Format("第{0}次 {1} 下料扫码条码为 {2}", index, strStep, barcode), LogLevels.Info, "Task_UnLoadScan");
                            string[] arrBarcode = barcode.Split(',');
                            barcode = arrBarcode[0];
                            if (barcode != string.Empty && !barcode.Contains("ERROR") && (barcode == queueSortingStation.Peek().Sn || barcode == queueSortingStation.Peek().LengthBarcode))
                            {
                                //条码一致
                                BatterySeat bt = queueSortingStation.Dequeue();

                                //TODO:把扫到的条码给到PLC，由于PLC字符串显示的高低位与上位机相反，在这里做下处理（上位机"1234"->PLC"2143"） zxh 2021-04-19
                                string revecode = UpLowReverse(barcode);
                                plc.WriteByVariableName("LaterBarcode", revecode);
                                WriteLog(string.Format("第{0}次 写入下料条码数据 {1}，处理后 {2}", index, barcode, revecode), LogLevels.Info, "Task_UnLoadScan");

                                if (plc.ReadByVariableName("NOOCV") == "0" && bt.OCVResult == false)
                                {
                                    if (bt.VoltageResult == false)
                                    {
                                        plc.WriteByVariableName("ScanVoltageNG", 1);
                                        WriteLog(string.Format("第{0}次 发送PLC下料结果校验 电压NG", index), LogLevels.Info, "Task_UnLoadScan");
                                    }
                                    else if (bt.ResistanceResult == false)
                                    {
                                        plc.WriteByVariableName("ScanResistanceNG", 1);
                                        WriteLog(string.Format("第{0}次 发送PLC下料结果校验 内阻NG", index), LogLevels.Info, "Task_UnLoadScan");
                                    }
                                    else if (bt.KValueResult == false)
                                    {
                                        plc.WriteByVariableName("ScanKNG", 1);
                                        WriteLog(string.Format("第{0}次 发送PLC下料结果校验 K值NG", index), LogLevels.Info, "Task_UnLoadScan");
                                    }
                                    else
                                    {
                                        //温度NG当其他NG处理
                                        plc.WriteByVariableName("ScanOtherNG", 1);
                                        WriteLog(string.Format("第{0}次 发送PLC下料结果校验 其他NG", index), LogLevels.Info, "Task_UnLoadScan");
                                    }
                                }
                                ...
                                {
                                    plc.WriteByVariableName("ScanXRAYNG", 1);
                                    WriteLog(string.Format("第{0}次 发送PLC下料结果校验 XRAYNG", index), LogLevels.Info, "Task_UnLoadScan");
                                }
                                else if (plc.ReadByVariableName("NOMDI") == "0" && bt.DimensionResult == false && CheckParamsConfig.Instance.IsNoUpLoadMdiAndPPGData == false)
                                {
                                    plc.WriteByVariableName("ScanMDING", 1);
                                    WriteLog(string.Format("第{0}次 发送PLC下料结果校验 FQING", index), LogLevels.Info, "Task_UnLoadScan");
                                }
                                else if (bt.IsMarkingCell == true)
                                {
                                    plc.WriteByVariableName("ScanOtherNG", 1);
                                    WriteLog(string.Format("第{0}次 {1} 是marking拦截电池，发送PLC下料结果校验 其他NG", index, bt.Sn), LogLevels.Info, "Task_UnLoadScan");
                                }
                                else if ((bt.KnifeNoConduction == true || bt.NeedleNoConduction == true) && plc.ReadByVariableName("NOIV") == "0")//不导通
                                {
                                    plc.WriteByVariableName("ScanIVNoConduction", 1);
                                    WriteLog(string.Format("第{0}次 IV 不导通 发送PLC下料结果校验 未导通NG", index), LogLevels.Info, "Task_UnLoadScan");
                                }
                                ...
                                {
                                    plc.WriteByVariableName("ScanIVNG", 1);
                                    WriteLog(string.Format("第{0}次 发送PLC下料结果校验 IVNG", index), LogLevels.Info, "Task_UnLoadScan");
                                }
                                else if (plc.ReadByVariableName("NOPPG") == "0" && bt.ThicknessResult == false && CheckParamsConfig.Instance.IsNoUpLoadMdiAndPPGData == false)
                                {
                                    plc.WriteByVariableName("ScanThicknessNG", 1);
                                    WriteLog(string.Format("第{0}次 发送PLC下料结果校验 PPGNG", index), LogLevels.Info, "Task_UnLoadScan");
                                }
                                else if (bt.MesResult == false)
                                {
                                    plc.WriteByVariableName("ScanOtherNG", 1);
                                    WriteLog(string.Format("第{0}次 MESNG,发送PLC下料结果校验 其他NG", index), LogLevels.Info, "Task_UnLoadScan");
                                }
                                else
                                {
                                    plc.WriteByVariableName("ScanResultOK", 1);
                                    WriteLog(string.Format("第{0}次 发送PLC下料结果校验 OK", index), LogLevels.Info, "Task_UnLoadScan");
                                }

                                plc.WriteByVariableName("BatteryScanning1OK", 1);
                                WriteLog(string.Format("第{0}次 释放电池对象，条码 {1}", index, bt.Sn), LogLevels.Info, "Task_UnLoadScan");
                                bt.Destroy();
                            }
                            else if (barcode.Contains("ERROR"))
                            {
                                ...
                                if (ngcounter >= 4)
                                {
                                    ...
                                    plc.WriteByVariableName("BatteryScanning1NG", 1);
                                    BatterySeat bt = queueSortingStation.Dequeue();
                                    WriteLog(string.Format("第{0}次 扫码失败，释放电池对象，条码 {1}", bt.TempBarcode, bt.Sn), LogLevels.Info, "Task_UnLoadScan");
                                    bt.Destroy();
                                }
                            }
                            MotionEnum.EnumUnLoadScan = MotionEnum.UnLoadScan.下料扫码完成;
                        }
                        //else if (queueSortingStation.Count > 10)
                        //{
                        //    BatterySeat bt = queueSortingStation.Dequeue();
                        //    WriteLog(string.Format("第{0}次 手动释放电池对象，条码 {1}", bt.TempBarcode, bt.Sn), LogLevels.Info, "Task_UnLoadScan");
                        //    bt.Destroy();
                        //}
                        break;
                    case MotionEnum.UnLoadScan.下料扫码完成:

                        if (plc.ReadByVariableName("ScanBarcodeSignal1") == "0")
                        {
                            plc.WriteByVariableName("ScanResultOK", 0);
                            plc.WriteByVariableName("ScanIVNG", 0);
                            plc.WriteByVariableName("ScanIVNoConduction", 0);
                            plc.WriteByVariableName("ScanOtherNG", 0);
                            plc.WriteByVariableName("ScanVoltageNG", 0);
                            plc.WriteByVariableName("ScanResistanceNG", 0);
                            plc.WriteByVariableName("ScanKNG", 0);
                            plc.WriteByVariableName("ScanMDING", 0);
                            plc.WriteByVariableName("ScanThicknessNG", 0);
                            plc.WriteByVariableName("ScanXRAYNG", 0);
                            plc.WriteByVariableName("BatteryScanning1OK", 0);
                            plc.WriteByVariableName("BatteryScanning1NG", 0);
                            WriteLog(string.Format("第{0}次 {1}", index, strStep), LogLevels.Info, "Task_UnLoadScan");
                            MotionEnum.EnumUnLoadScan = MotionEnum.UnLoadScan.下料扫码触发;
                        }
                        break;
                }
            ...
            while (true)
            {
                ...
                switch (MotionEnum.EnumXrayShot1)
                {
                    case MotionEnum.XRAYShot1.触发角一拍照:
                        ...
                        if (plc.ReadByVariableName("Battery1CornerPhotoSignal") == "1" && plc.ReadByVariableName("TestMode") == "1")
                        {
                            ...
                            System.Windows.MessageBoxResult.OK, System.Windows.MessageBoxOptions.ServiceNotification);
                                ...
                                if (canDoXRAYNeedle == false)
                                {
                                    System.Windows.MessageBox.Show("针规未点检完成，不能启动XRAY功能", "提示", System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.None,
                            System.Windows.MessageBoxResult.OK, System.Windows.MessageBoxOptions.ServiceNotification);
                                    Thread.Sleep(1000);
                                    continue;
                                }
                    ...
                    case MotionEnum.XRAYShot1.角一拍照:
                        ...
                        if (Math.Abs(XRayTubeIF.XRayTube1Stauts.ActualVoltage - HardwareConfig.Instance.XRayConfig1.PresetVoltage) < 2 && Math.Abs(XRayTubeIF.XRayTube1Stauts.ActualCurrent - HardwareConfig.Instance.XRayConfig1.PresetCurrent) < 2)
                        {
            ...
            while (true)
            {
                ...
                switch (MotionEnum.EnumXrayShot2)
                {
                    ...
                    case MotionEnum.XRAYShot2.角三拍照:
                        ...
                        if (Math.Abs(XRayTubeIF.XRayTube2Status.ActualVoltage - HardwareConfig.Instance.XRayConfig2.PresetVoltage) < 2 && Math.Abs(XRayTubeIF.XRayTube2Status.ActualCurrent - HardwareConfig.Instance.XRayConfig2.PresetCurrent) < 2)
                        {
                            try
                            {
                    ...
                    case MotionEnum.XRAYShot2.触发角四拍照:
                        ...
                        if (plc.ReadByVariableName("Battery4CornerPhotoSignal") == "1")
                        {
                            ...
                            System.Windows.MessageBoxResult.OK, System.Windows.MessageBoxOptions.ServiceNotification);
                                ...
                                if (ReworkMode == 4 || ReworkMode == 5 || ReworkMode == 6 || ReworkMode == 9)//isOpenBis == false
                                {
                                    bt.CheckExtension = ECheckExtensions.Local;//本地模式不保存数据到数据库
                                }

                                //上传OCV、MDI数据
                                if (isMasterCell == false)
                                {
                                    if ((ReworkMode == 0 || ReworkMode == 1 || ReworkMode == 2 || ReworkMode == 5 || ReworkMode == 7 || ReworkMode == 8) && string.IsNullOrWhiteSpace(bt.MasterType) && plc.ReadByVariableName("NOOCV") == "0")
                                    {
                                        double Resistance = 0.000;
                                        if (double.TryParse(bt.Resistance.ToString(), out Resistance))//OCV 未测到数据不上传
                                        {
                    ...
                    case MotionEnum.XRAYShot2.处理XRAY数据:
                        ...
                        if (m_bAlgoCorner1Finished && m_bAlgoCorner2Finished)
                        {
                            ...
                            //    UpLoadData(ref bt, "OCV");
                            //}
                            double Resistance = 0.000;//内阻测试到正常数值
                            if (ReworkMode != 8 || ReworkMode != 6)
                            {
                                string strOut;
                                string testType = "IV";
                                int result = -2;
                                if (plc.ReadByVariableName("NOXRAY") == "0")
                                {
                                    ...
                                    WriteLog(string.Format($"第{index}次,条码{bt.Sn} 合成结果图完成,电压{bt.Voltage},电压结果{bt.VoltageResult},内阻{bt.Resistance},内阻结果{bt.ResistanceResult},OCV测试结果{bt.OCVResult}"), LogLevels.Info, "Task_XrayShot2");
                                    ...
                                    if ((bt.ResultCode == EResultCodes.Unknow || bt.ResultCode == EResultCodes.AlgoErr) && plc.ReadByVariableName("NOXRAY") == "0")
                                    {
                                        try
                                        {
                                            ...
                                            {
                                                MessageBox.Show("请检查硬盘内存空间" + ex.ToString());
                                                continue;
                                            }
                                    ...
                                    try
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                MessageBox.Show("请检查内存与Excel是否打开" + ex.ToString());
                                            }
                                        }
                                    }
                                    catch (Exception ex)
                                    ...
                                    {
                                        if (ReworkMode == 0 || ReworkMode == 1 || ReworkMode == 2 || ReworkMode == 7)
                                        {
                                            if (bt.MesOCVResult && double.TryParse(bt.Resistance.ToString(), out Resistance))
                                            {
                                                //上传XRAY数据
                                                UpLoadData(ref bt, "XRAY");
                                            }
                                        }
                                        else if (ReworkMode == 3 && double.TryParse(bt.Resistance.ToString(), out Resistance))
                                        {
                                            UpLoadData(ref bt, "XRAY");
                                        }
                                    }
                                ...
                                {
                                    if (bt.MesOCVResult && bt.MesXRAYResult && (ReworkMode == 0 || ReworkMode == 1) && double.TryParse(bt.Resistance.ToString(), out Resistance))
                                    {
                                        UpLoadData(ref bt, "FQI");
                                    }
                                    else if (bt.MesXRAYResult && (ReworkMode == 3 || ReworkMode == 2) && double.TryParse(bt.Resistance.ToString(), out Resistance))
                                    {
                                        UpLoadData(ref bt, "FQI");
                                    }
                                    else if (bt.MesOCVResult && (ReworkMode == 4 || ReworkMode == 5) && double.TryParse(bt.Resistance.ToString(), out Resistance))
                                    {
                                        UpLoadData(ref bt, "FQI");
                                    }
                                    else if (ReworkMode == 6)
        ...
        private void DataHandle(ref BatterySeat bt, int index)
        {
            ...
            //上传OCV、NDI数据
            if (plc.ReadByVariableName("NOOCV") == "0" && (ReworkMode == 0 || ReworkMode == 1 || ReworkMode == 4 || ReworkMode == 5 || ReworkMode == 7))
            {
                UpLoadData(ref bt, "OCV");
            }
            if (ReworkMode != 8 || ReworkMode != 6)
            {
                ...
                if (plc.ReadByVariableName("NOXRAY") == "0")
                {
                    ...
                    if (ReworkMode == 4 || ReworkMode == 5 || ReworkMode == 6 || ReworkMode == 7 || ReworkMode == 9)//isOpenBis == false
                    {
                        bt.CheckExtension = ECheckExtensions.Local;//本地模式不保存数据到数据库
                    }
                ...
                if (plc.ReadByVariableName("NOXRAY") == "0" && CheckParamsConfig.Instance.IsNoUpLoadMdiAndPPGData == false)
                {
                    ...
                    else if (bt.MesXRAYResult && (ReworkMode == 3 || ReworkMode == 2))
                    {
                        UpLoadData(ref bt, "FQI");
                    }
                    else if (bt.MesOCVResult && (ReworkMode == 4 || ReworkMode == 5))
                    {
                        UpLoadData(ref bt, "FQI");
                    }
                    else if (ReworkMode == 6)
                ...
                switch (MotionEnum.EnumSorting)
                {
                    case MotionEnum.Sorting.触发分拣:
                        ...
                        if (plc.ReadByVariableName("PhotoResultSignal") == "1" && plc.ReadByVariableName("TestMode") == "1")
                        {
                            ...
                            if (bt.MasterType != "")
                            {
                                //TODO: 记录Master IV点检电芯 ZhangKF 2021-4-6
                                IVSeriesCheck.Record(bt);
                                ...
                                if (bt.MasterType.ToUpper().Contains("IV"))
                                {
                                    ...
                                    {
                                        //连续4个探针不通，连续4个刺刀不通，连续4个IV值超规格，都满足才点检通过
                                        ivCheckCount++;
                                        //TODO:修改IV点检NG电芯数量 ZhangKF 2021-4-6
                                        //if (ivCheckCount == 4)
                                        if (ivCheckCount >= IVSeriesCheck.MAX_NG_BATTERY_COUNT && IVSeriesCheck.CheckPassed())
                                        {
                            ...
                            if (ReworkMode == 6)//只判断FQI
                            {
                                ...
                                else
                                {
                                    ...
                                    plc.WriteByVariableName("ResultOK", 1);
                                    WriteLog(string.Format("第{0}次 OCVG发送PLC分拣 OK", index), LogLevels.Info, "Task_Sorting");
                                }
                            }
                            else
                            if (ReworkMode == 8)//只判断OCV
                            {
                                if (plc.ReadByVariableName("NOOCV") == "0" && bt.OCVResult == false)
                                {
                                    if (bt.VoltageResult == false)
                                    ...
                                    else if (bt.KValueResult == false)//isKValueNG == true
                                    {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    if (Common.IsWeekCheck)//客户要求周次抽检OK品
                                    {
                                        //条码第5、6位为周次
                                        if (bt.Sn.Length >= 6)
                                        {
                                            string week = bt.Sn.Substring(4, 2);
                                            int weeks = 0;
                                            if (!int.TryParse(week, out weeks))
                                            {
                                                string errmsg = "条码第5、6位不为数字，请关闭周次校验!";
                                                System.Windows.MessageBox.Show(errmsg, "提示",
                                                System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.Information, System.Windows.MessageBoxResult.OK, System.Windows.MessageBoxOptions.ServiceNotification);
                                            }
                                            if (weeks.Equals(Common.WeekCheck))//周次达到条件
                                            {
                                                if (checkCount < Common.WeekCheckCount)//周次个数
                                                {
                                                    checkCount++;
                                                    #region   A085接口,随机取样
                                                    List<string> SERIALNOS = new List<string>();
                                                    SERIALNOS.Add(bt.Sn);
                                                    List<string> DEFECTCODES = new List<string>();
                                                    DEFECTCODES.Add("SSJQYA");
                                                    ATL.MES.A086.Root root_86 = InterfaceClient.Current.A085(SERIALNOS, DEFECTCODES, "G1804");
                                                    ...
                                                    Common.SetRichText(rtb, bt.Sn);
                                                    #endregion
                                                    plc.WriteByVariableName("WeekCheck", 1);
                                                    WriteLog(string.Format($"第{index}次 周次抽检条码{bt.Sn}"), LogLevels.Info, "Task_Sorting");
                                                }
                                                else
                                                {
                                                    BotIF.MyCheckStatus.MyStatistics.OKNum++;
                            ...
                            if (bt.IsMarkingCell == false)
                            {
                                if (plc.ReadByVariableName("NOOCV") == "0" && bt.OCVResult == false)
                                {
                                    if (!bt.VoltageResult || !bt.ResistanceResult || !bt.KValueResult)
                                    {
                    ...
                    case MotionEnum.Sorting.分拣完成:
                        ...
                        if (plc.ReadByVariableName("PhotoResultSignal") == "0")
                        {
                            ...
                            bool ok = plc.WriteByVariableName("ResultOK", 0);
                            if (!ok) plc.WriteByVariableName("ResultOK", 0);

                            ok = plc.WriteByVariableName("IVNG", 0);
                            if (!ok) plc.WriteByVariableName("IVNG", 0);

                            ok = plc.WriteByVariableName("WeekCheck", 0);
                            if (!ok) plc.WriteByVariableName("WeekCheck", 0);

                            ok = plc.WriteByVariableName("IVNoConduction", 0);
                            if (!ok) plc.WriteByVariableName("IVNoConduction", 0);

                            ok = plc.WriteByVariableName("OtherNG", 0);
                            if (!ok) plc.WriteByVariableName("OtherNG", 0);

                            ok = plc.WriteByVariableName("VoltageNG", 0);
                            if (!ok) plc.WriteByVariableName("VoltageNG", 0);

                            ok = plc.WriteByVariableName("ResistanceNG", 0);
                            if (!ok) plc.WriteByVariableName("ResistanceNG", 0);

                            ok = plc.WriteByVariableName("KNG", 0);
                            if (!ok) plc.WriteByVariableName("KNG", 0);

                            ok = plc.WriteByVariableName("MDING", 0);
                            if (!ok) plc.WriteByVariableName("MDING", 0);

                            ok = plc.WriteByVariableName("ThicknessNG", 0);
                            if (!ok) plc.WriteByVariableName("ThicknessNG", 0);

                            ok = plc.WriteByVariableName("XRAYNG", 0);
                            if (!ok) plc.WriteByVariableName("XRAYNG", 0);

                            DataViewPage.Instance.UpdateListView(bt);

                            WriteLog(string.Format("第{0}次 条码 {1} 入下料扫码队列 {2}", index, bt.Sn, queueSortingStation.Count), LogLevels.Info, "Task_Sorting");
                            queueSortingStation.Enqueue(bt);
            ...
            while (true)
            {
                ...
                switch (MotionEnum.EnumOCV1)
                {
                    ...
                    case MotionEnum.OCV1.工位1OCV电压测试完成:
                        ...
                        if (plc.ReadByVariableName("VoltageSignal1") == "0" && isOcv2Finished == true)
                        {
                            EnterQueue(bt1, "OCV1");
                ...
                switch (MotionEnum.EnumMDI1)
                {
                    case MotionEnum.MDI1.工位1触发FQI检测:
                        ...
                        else if (plc.ReadByVariableName("MDISignal3") == "1" && (plc.ReadByVariableName("TestMode") == "1") && isCCDBusy == false)
                        {
                            ...
                            if (plc.ReadByVariableName("MDIPlatForm2CellCount") == "2" && plc.ReadByVariableName("NOPPG") == "1")
                            {
                    ...
                    case MotionEnum.MDI1.工位1FQI完成:
                        ...
                        if (plc.ReadByVariableName("MDISignal1") == "0" && plc.ReadByVariableName("MDISignal3") == "0")
                        {
                            ...
                            //TODO:根据尺寸测量时的记录，已经发生尺寸与上次测量一模一样的异常，需要弹出对话框提醒用户 ZhangKF 2021-4-9
                            if (_MessageBox_Same_Exception_MDIComplete1)
                            {
                                MessageBox.Show(_MessageBox_Msg_MDIComplete1, "提示", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Information, System.Windows.Forms.MessageBoxDefaultButton.Button1, System.Windows.Forms.MessageBoxOptions.ServiceNotification);
                                _MessageBox_Same_Exception_MDIComplete1 = false;
                            }
                            //End
                        }
                ...
                switch (MotionEnum.EnumMDI2)
                {
                    case MotionEnum.MDI2.工位2触发FQI检测:
                        ...
                        else if (plc.ReadByVariableName("MDISignal4") == "1" && (plc.ReadByVariableName("TestMode") == "1") && isCCDBusy == false)
                        {
                            ...
                            {
                                EnterQueue(bt, "MDI2");
                                EnterQueue(queueNoPPGOfMDIProduct.Dequeue(), "MDI1");
                            }
                            else
                            {
                                EnterQueue(bt, "MDI2");
                    ...
                    case MotionEnum.MDI2.工位2FQI完成:
                        ...
                        if (plc.ReadByVariableName("MDISignal2") == "0" && plc.ReadByVariableName("MDISignal4") == "0")
                        {
                            ...
                            //TODO:根据尺寸测量时的记录，已经发生尺寸与上次测量一模一样的异常，需要弹出对话框提醒用户 ZhangKF 2021-4-9
                            if (_MessageBox_Same_Exception_MDIComplete2)
                            {
                                MessageBox.Show(_MessageBox_Msg_MDIComplete2, "提示", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Information, System.Windows.Forms.MessageBoxDefaultButton.Button1, System.Windows.Forms.MessageBoxOptions.ServiceNotification);
                                _MessageBox_Same_Exception_MDIComplete2 = false;
                            }
                        }
            ...
            while (true)
            {
                ...
                switch (MotionEnum.EnumThickness1)
                {
                    case MotionEnum.Thickness1.工位1触发测厚:
                        ...
                        if (plc.ReadByVariableName("ThicknessMeasureSignal1") == "1" && plc.ReadByVariableName("TestMode") == "1")
                        {
                            ...
                            System.Windows.MessageBoxResult.OK, System.Windows.MessageBoxOptions.ServiceNotification);
                                ...
                                //TODO: 厚度线性点检未通过,不允许继续测试 2021-4-5 ZhangKF
                                if (!canDoPPGLinear)
                                {
                                    System.Windows.MessageBox.Show("电芯厚度线性未点检完成，不能启动测厚功能", "提示", System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.None,
                                    System.Windows.MessageBoxResult.OK, System.Windows.MessageBoxOptions.ServiceNotification);
                                    Thread.Sleep(3000);
                                    continue;
                                }
                            ...
                            else
                            {
                                ...
                                if (ppgNGCounts1 >= CheckParamsConfig.Instance.PPGWarmingCounts)
                                {
                                    ...
                                    //    stressPage.ShowInTaskbar = false;
                    ...
                    case MotionEnum.Thickness1.工位1测厚完成:
                        ...
                        //后面的工位变为单通道，必须要等待工位2先完成
                        if (plc.ReadByVariableName("ThicknessMeasureSignal1") == "0" && plc.ReadByVariableName("ThicknessMeasureSignal3") == "0" && isPPG2Finished == true)
                        {
                            //TODO: 增加延时，更加确保1工位在2工位之后入队列 ZhangKF 2021-4-7
                            Thread.Sleep(100);
                    ...
                    case MotionEnum.RemoveProduct.触发移除电池:
                        ...
                        if (plc.ReadByVariableName("RemoveBatterySignal") == "1")
                        {
                            ...
                            if (RemoveProduct(tempBarcode, ref queueScanStation1) || RemoveProduct(tempBarcode, ref queueScanStation2) ||
                                ...
                                RemoveProduct(tempBarcode, ref queueSortingStation))
                            {
                                Thread.Sleep(1500);
        ...
        private bool GetMdiData(ref BatterySeat bt, int time, int position, bool isGrr = false)
        {
            try
            {
                ...
                {
                    //TODO:将弹出位置放置在完成信号(MDIComplete1/2)归零后，再弹出 ZhangKF 2021-4-9
                    //string errmsg = "尺寸数据跟上次的一模一样，请确认！";
                    //MessageBox.Show(errmsg, "提示", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Information, System.Windows.Forms.MessageBoxDefaultButton.Button1, System.Windows.Forms.MessageBoxOptions.ServiceNotification);
                    //WriteLog(errmsg, LogLevels.Warn, "GetFQIData");
                    if (bt.MDIChannel == 1 || bt.MDIChannel == 3)
                    {
                        _MessageBox_Same_Exception_MDIComplete1 = true;
                        _MessageBox_Msg_MDIComplete1 = string.Format("尺寸测量平台1的工位{0}与上次测值一模一样!", position);
                    }
                    else if (bt.MDIChannel == 2 || bt.MDIChannel == 4)
                    {
                        _MessageBox_Same_Exception_MDIComplete2 = true;
                        _MessageBox_Msg_MDIComplete2 = string.Format("尺寸测量平台2的工位{0}与上次测值一模一样!", position);
                    }
                    //End TODO
                    ...
                    if (Common.FourThickness)
                    {
                        switch (position)
                        {
                            case 1:
                                ...
                                break;
                            case 2:
                                thicknessValue = (thicknessValue * CheckParamsConfig.Instance.ThicknessKValue + CheckParamsConfig.Instance.ThicknessBValue) * CheckParamsConfig.Instance.CellKValue2 + CheckParamsConfig.Instance.CellBValue2;
                                break;
                            case 3:
                                thicknessValue = (thicknessValue * CheckParamsConfig.Instance.ThicknessKValue + CheckParamsConfig.Instance.ThicknessBValue) * CheckParamsConfig.Instance.CellKValue3 + CheckParamsConfig.Instance.CellBValue3;
                                break;
                            case 4:
                                thicknessValue = (thicknessValue * CheckParamsConfig.Instance.ThicknessKValue + CheckParamsConfig.Instance.ThicknessBValue) * CheckParamsConfig.Instance.CellKValue4 + CheckParamsConfig.Instance.CellBValue4;
                                break;
                            default:
                                break;
                        }
                    ...
                    else
                    {
                        ...
                        switch (position)
                        {
                            case 1:
                                ...
                                if (listPPGdata1.Count > CheckParamsConfig.Instance.StationRangeNum)
                                {
                            ...
                            case 2:
                                ...
                                if (listPPGdata2.Count > CheckParamsConfig.Instance.StationRangeNum)
                                {
                            ...
                            case 3:
                                ...
                                if (listPPGdata3.Count > CheckParamsConfig.Instance.StationRangeNum)
                                {
                            ...
                            case 4:
                                ...
                                if (listPPGdata4.Count > CheckParamsConfig.Instance.StationRangeNum)
                                {
                                    listPPGdata4.RemoveAt(0);
                                }
                                break;
                        }
                        if (listPPGdata1.Count >= CheckParamsConfig.Instance.StationRangeNum && listPPGdata2.Count >= CheckParamsConfig.Instance.StationRangeNum
                            && listPPGdata3.Count >= CheckParamsConfig.Instance.StationRangeNum && listPPGdata4.Count >= CheckParamsConfig.Instance.StationRangeNum)
                        {
                            ...
                            {
                                string errmsg;
                                listPPGOutTime.Add(DateTime.Now);
                                WriteLog(string.Format("第{0}次厚度极差报警，报警时间 {1}", index, DateTime.Now.ToString("HH:mm:ss")), LogLevels.Warn, "GetTickness");
                                if (CheckParamsConfig.Instance.StationRangeOutTimes >= 2 && listPPGOutTime.Count >= CheckParamsConfig.Instance.StationRangeOutTimes)
                                {
                                    TimeSpan ts = listPPGOutTime.Last() - listPPGOutTime.First();
                                    if (ts.TotalMinutes <= 60)
                                    {
                                        errmsg = string.Format("系统检测到一个小时内出现{0}次厚度极差报警，请停机检查！", CheckParamsConfig.Instance.StationRangeOutTimes);
                                        App.Current.Dispatcher.Invoke(delegate ()
                                        {
                                            ...
                                            stressPage.Closed += new EventHandler(stressPage_Closed);
                                            stressPage.ShowInTaskbar = false;
                                            //stressPage.Topmost = true;
                                            stressPage.ShowDialog();
                                        });
                                    }
                                    listPPGOutTime.RemoveAt(0);
                                }
                                errmsg = "4个测厚平台各自的平均值极差为：" + (avg.Max() - avg.Min()) + " 超过" + CheckParamsConfig.Instance.StationRange + "mm"
                ...
                switch (currentStation)
                {
                    case "BarcodeBinding1":
                        if (plc.ReadByVariableName("NOPPG") == "1" && plc.ReadByVariableName("NOMDI") == "1" && plc.ReadByVariableName("NOOCV") == "1" && plc.ReadByVariableName("NOIV") == "1")
                        {
                            ...
                            WriteLog(string.Format(" {0} <==> {1} 从 {2} 入PPG队列", bt.TempBarcode, bt.Sn, currentStation), LogLevels.Info, "EnterQueue");
                        }
                        else if (plc.ReadByVariableName("NOMDI") == "1" && plc.ReadByVariableName("NOOCV") == "1" && plc.ReadByVariableName("NOIV") == "1")
                        {
                            ...
                            WriteLog(string.Format(" {0} <==> {1} 从 {2} 入MDI1队列", bt.TempBarcode, bt.Sn, currentStation), LogLevels.Info, "EnterQueue");
                        }
                        else if (plc.ReadByVariableName("NOOCV") == "1" && plc.ReadByVariableName("NOIV") == "1")
                        {
                            queueOCVStation1.Enqueue(bt);
                            WriteLog(string.Format(" {0} <==> {1} 从 {2} 入OCV1队列", bt.TempBarcode, bt.Sn, currentStation), LogLevels.Info, "EnterQueue");
                        }
                        else if (plc.ReadByVariableName("NOIV") == "1")
                        {
                            queueIVStation1.Enqueue(bt);
                            WriteLog(string.Format(" {0} <==> {1} 从 {2} 入IV1队列", bt.TempBarcode, bt.Sn, currentStation), LogLevels.Info, "EnterQueue");
                        }
                        else
                        {
                            queueScanStation1.Enqueue(bt);
                            WriteLog(string.Format(" {0} <==> {1} 从 {2} 入扫码1队列", bt.TempBarcode, bt.Sn, currentStation), LogLevels.Info, "EnterQueue");
                        }
                        break;
                    ...
                    case "BarcodeBinding2":
                        ...
                        else if (plc.ReadByVariableName("NOMDI") == "1" && plc.ReadByVariableName("NOOCV") == "1" && plc.ReadByVariableName("NOIV") == "1")
                        {
                            ...
                            WriteLog(string.Format(" {0} <==> {1} 从 {2} 入MDI2队列", bt.TempBarcode, bt.Sn, currentStation), LogLevels.Info, "EnterQueue");
                        }
                        else if (plc.ReadByVariableName("NOOCV") == "1" && plc.ReadByVariableName("NOIV") == "1")
                        {
                            queueOCVStation2.Enqueue(bt);
                            WriteLog(string.Format(" {0} <==> {1} 从 {2} 入OCV2队列", bt.TempBarcode, bt.Sn, currentStation), LogLevels.Info, "EnterQueue");
                        }
                        else if (plc.ReadByVariableName("NOIV") == "1")
                        {
                            queueIVStation2.Enqueue(bt);
                            WriteLog(string.Format(" {0} <==> {1} 从 {2} 入IV2队列", bt.TempBarcode, bt.Sn, currentStation), LogLevels.Info, "EnterQueue");
                        }
                        else
                        {
                            queueScanStation2.Enqueue(bt);
                            WriteLog(string.Format(" {0} <==> {1} 从 {2} 入扫码2队列", bt.TempBarcode, bt.Sn, currentStation), LogLevels.Info, "EnterQueue");
                        }
                        break;
                    ...
                    case "IV3":
                        ...
                        {
                            queueXRAY2Station.Enqueue(bt);
                            WriteLog(string.Format(" {0} <==> {1} 从 {2} 入XRAY2队列", bt.TempBarcode, bt.Sn, currentStation), LogLevels.Info, "EnterQueue");
                        }
                ...
                if (sourceData.Contains("!"))
                {
                    ...
                    if (list.Count > 0)
                    {
                        for (int i = 0; i < list.Count; i++)
                        {
                            if (DateTime.Now >= dayWorkStart && DateTime.Now <= dayWorkEnd) //白班
                            {
                                    ...
                                    Convert.ToDateTime(list[i].CheckTime) <= dayWorkEnd)
                                {
                                    ...
                                    if (list[i].Result == "OK" && list[i].Poisition == "4")
                                    {
                                        if (list[i].Modle.Contains("小"))
                                        {
                                            smallOKCount4++;
                        ...
                        if (smallOKCount1 >= checkCount && midleOKCount1 >= checkCount && bigOKCount1 >= checkCount && smallOKCount2 >= checkCount && midleOKCount2 >= checkCount && bigOKCount2 >= checkCount
                                && smallOKCount3 >= checkCount && midleOKCount3 >= checkCount && bigOKCount3 >= checkCount && smallOKCount4 >= checkCount && midleOKCount4 >= checkCount && bigOKCount4 >= checkCount)
                        {
                            isChecked = true;
                        }
                        else
        ...
        public bool CheckCanDo(string checkTime)
        {
            try
            {
                ...
                //早晚七点半到十二点可以直接开机
                if ((DateTime.Now > dayWorkStart && DateTime.Now < dayWorkStart.AddHours(4.5)) || (DateTime.Now > dayWorkEnd && DateTime.Now < dayWorkEnd.AddHours(4.5)))
                {
                    return true;
                }
        ...
        ////TODO:增加厚度线性点检是否通过判定 ZhangKF 2021-4-5
        /////<summary>检查线性点检是否可用</summary>
        //public bool CheckLinerCanDo()
        //{
        //    if (XRayTubeIdleTime == 9999)
        //    {
        //        return true;
        ...
        //    DateTime checkRangeStart = new DateTime(1900, 1, 1);
        //    if (!DateTime.TryParse(CheckParamsConfig.Instance.LinearCheckStartTime, out checkRangeStart))
        //    {
        //        return pass;
        //    }

        //    DateTime checkRangeEnd = new DateTime(1900, 1, 1);
        //    if (!DateTime.TryParse(CheckParamsConfig.Instance.LinearCheckEndTime, out checkRangeEnd))
        //    {
        //        return pass;
        //    }

        //    //当前时间，在点检对应的班次时间范围内且已达到点检要求的次数
        //    if (DateTime.Now >= checkRangeStart && DateTime.Now <= checkRangeEnd && CheckParamsConfig.Instance.LinearOKNum >= CheckParamsConfig.Instance.LinearNeedOK)
        //    {
        //        pass = true;
        //    }
        ...
        private void UpLoadData(ref BatterySeat bt, string name, bool isRecheck = false)
        {
            ...
            if (Common.FactoryNo.Equals("W") && name == "FQI")
            {
                name = "MDI";
            }
            string json = string.Empty;
            ...
            else
            {
                ...
                WriteLog(string.Format($"第{ bt.TempBarcode}次 条码:{bt.Sn},电压:{bt.Voltage},电压结果{bt.VoltageResult},内阻{bt.Resistance},内阻结果{bt.ResistanceResult},OCV结果{bt.OCVResult},上传 {name} MES成功(A013接口), 耗时 = { (endTime1 - startTime1).TotalMilliseconds} 毫秒,返回值：{ headerResult} ,errmsg= {root_14.Header.ErrorMsg}"), LogLevels.Info, "UpLoadData");
