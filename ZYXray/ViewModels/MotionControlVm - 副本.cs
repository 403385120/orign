...
namespace ZYXray.ViewModels
{
    public class MotionControlVm : ObservableObject
    {
        ...
        private static List<BatterySeat> listSortingStation = new List<BatterySeat>();
                ...
                switch (MotionEnum.EnumUnLoadScan)
                {
                    case MotionEnum.UnLoadScan.下料扫码触发:
                        ...
                        if (plc.ReadByVariableName("ScanBarcodeSignal1") == "1" && plc.ReadByVariableName("TestMode") == "1")
                        {
                            ...
                            BatterySeat bt = listSortingStation.FirstOrDefault(s => s.Sn == barcode || s.LengthBarcode == barcode);
                            if (bt == null)
                            {
                                WriteLog("下料扫码找不到对应条码电芯!", LogLevels.Warn, "Task_UnLoadScan");
                                Thread.Sleep(3000);
                                continue;
                            }

                            index = bt.TempBarcode;
                            //WriteLog(string.Format("第{0}次 {1} 下料扫码条码为 {2}", queueSortingStation.Peek().TempBarcode, strStep, barcode), LogLevels.Info, "Task_UnLoadScan");
                            WriteLog(string.Format("第{0}次 {1} 下料扫码条码为 {2}", bt.TempBarcode, strStep, barcode), LogLevels.Info, "Task_UnLoadScan");
                            if (barcode != string.Empty && !barcode.Contains("ERROR")/* && (barcode == queueSortingStation.Peek().Sn || barcode == queueSortingStation.Peek().LengthBarcode)*/)
                            {
                                ...
                                //}
                                //else if (plc.ReadByVariableName("NOOCV") == "0" && bt.OCVResult == false)
                                //{
                                ...
                                //    WriteLog(string.Format("第{0}次 发送PLC下料结果校验 MDING", index), LogLevels.Info, "Task_UnLoadScan");
                                //}
